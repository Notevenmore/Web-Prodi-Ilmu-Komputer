...
<div class="h-1/2 w-full flex flex-col">
  <a href="{{ route('detail.berita', ['id' => 1]) }}">
    ...
    @for($i = 1; $i <= 4; $i++)
        <div class="flex flex-col w-full h-full">
          ...
          <div class="flex flex-col bg-[#D9D9D9] p-3 sm:p-6 gap-2 sm:gap-5">
            ...
            <a href="{{ route('detail.berita', ['id' => $i+1]) }}" class="text-white bg-biru p-3 lg:p-2 xl:p-3 w-full lg:w-3/4 2xl:w-1/2 xl:text-base text-[0.5rem] sm:text-base lg:text-[0.6rem] rounded-xl text-center">Baca Selengkapnya</a>
          </div>
        </div>
    @endfor
  </div>
