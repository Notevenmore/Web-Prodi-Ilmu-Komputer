@props(['title', 'marginBottom', 'fontSize', 'line', 'textColor', 'responsive', 'hidden', 'gap', 'position'])
<div  class="flex flex-col @isset($position) {{ $position }} @endif @isset($gap) {{ $gap }} @else gap-2 @endif" @isset($marginBottom) style="margin-bottom: {{ $marginBottom }}px" @endif>
